...
function settingsFor(p: LiquidGridProps | undefined, isDark: boolean) {
  const weight = clamp(p?.lineWidth, 1, 10, DEFAULTS.lineWidth);
  const defaultLineColor = isDark
    ? "rgba(120,120,120,0.22)"
    : "rgba(0,0,0,0.4)";
  const defaultGlowColor = isDark ? "#FFFFFF" : "#000000";

  return {
    mode: p?.mode === "dots" ? "dots" : "lines",
    background: p?.background ?? DEFAULTS.background,
    lineColor: p?.lineColor ?? defaultLineColor,
    glowColor: p?.glowColor ?? defaultGlowColor,
...
export default function LiquidGrid(props: LiquidGridProps) {
  ...
  const propKey =
    JSON.stringify(
      Object.keys(DEFAULTS).map((k) => (props as Record<string, unknown>)?.[k]),
    ) + isDark;
  ...
  useEffect(() => {
    ...
    function drawFrame(S: ReturnType<typeof settingsFor>) {
      const { W, H } = rip;
      if (!W || !H) return;
    ...
    const paint = () => {
      resize();
      drawFrame(settingsFor(propsRef.current, isDarkRef.current));
    };
    ...
    function onClick(e: MouseEvent) {
      const S = settingsFor(propsRef.current, isDarkRef.current);
      if (!S.click) return;

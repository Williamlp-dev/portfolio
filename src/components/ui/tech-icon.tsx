...
const TECH_ICON_MAP: Record<string, IconSource> = {
  ...
  Typescript: "/icons/typescript.svg",
  "Next.js": "/icons/nextjs_icon_dark.svg", // This could also be a light/dark object if you have nextjs_icon_light.svg, but we'll stick to the user's map.
  ...
  return (
    <>
      <Image
        alt={tech}
        className={cn(
          "dark:hidden shrink-0 transition-opacity duration-200",
          className,
        )}
        height={size}
        src={src.light}
      ...
      <Image
        alt={tech}
        className={cn(
          "hidden dark:block shrink-0 transition-opacity duration-200",
          className,
        )}
        height={size}
        src={src.dark}
